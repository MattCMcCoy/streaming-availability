...
export function FollowerList({ userId }: { userId: string }) {
  ...
  const {
    ...
    error
  } = api.follow.followers.useQuery({ userId });
  ...
  if (!following) return <div>You are not following anyone!</div>;
...
function BuildFollowListing({
  ...
  if (!user) {
    toast({
      title: `User not found: ${follow.followingId}`,
  ...
  return (
    <div className="ml-10 mt-5 flex flex-row items-center align-middle">
      ...
      <div className="ml-auto mr-10">
        <FollowButton userId={sessionUser} followId={follow.followingId} />
      </div>
    </div>
  );
}
